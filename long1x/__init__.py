...
class Long1x:
    ...
    def run(self):
        ...
        while self.socket.ws.sock.connected:
            ...
            if leverage <= lower_lev:
                qty = round((TARGET_LEVERAGE - leverage) / lev_per_pos)
                self.order.limit_order(qty, sell_price)
                # self.order.market_order(-qty)
            elif leverage > upper_lev:
                qty = round((leverage - TARGET_LEVERAGE) / lev_per_pos)
                self.order.market_order(-qty)
